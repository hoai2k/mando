...
reuses the delivered `duelist.glb`. Open on the model side: the hunter weapon props —
`saber_curved`, `crossbow`, `longrifle` (sheets delivered 2026-08-29, ready to model)
and `pistol` (sheet still requested in [`ASSETS_IMAGES.md`](ASSETS_IMAGES.md)) — plus
the whole environment batch above. A character who carries a pair
