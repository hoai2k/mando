...
- **Assets warmed ahead, on the screens nobody is playing.** Every heavy file is pulled down during the seconds a player spends in menus, in priority order and only while the browser reports itself idle, two downloads at a time so warming never starves the thing actually being waited on. The title screen warms the first Mandalorian and the nine territory cards; the territory grid warms the rest of the roster; choosing a territory warms *that* board's sky, ground textures and the models its opening waves post, with the later waves trailing behind at low priority. Every real load — models through the glTF loader's progress events, textures as they start and finish — reports to one tracker, so anything can ask what is still coming and how far along it is without being handed a list.
