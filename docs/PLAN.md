...
- **Player cover** (C / Y on the ground, near a box ≥1 m tall and ≥1 m wide, within ~2.4 m) — RDR2 snap-to-cover: the player presses against the box face (a HUD prompt appears in range), slides along it with the stick (clamped to the face), and is protected while tucked — enemy sightlines and bolts are blocked by the box, and the tucked player holds fire. Holding aim leans out past a corner to shoot: the corner is chosen by camera lean, overridden by whichever corner has a clear raycast to the soft-locked target (re-checked ~3×/s — crates sit in rows, and leaning into the neighbour is a peek wasted); the lean reaches ~1 m past the edge so shots clear deep boxes. Release tucks back in. Jump (exits with the jump), dash, melee, pressing cover again, or pushing off the wall for 0.2 s all leave. Weapon switching is disabled in cover; rockets fire only while leaned out.
- **Camera distance** (hold right-stick click, then push the stick up or down; mouse wheel on keyboard) — dollies the chase camera between 1.1 m and 11 m and *keeps* it there. It starts at 1.9 m — what used to be the closest the dolly could go — so the default view is over the shoulder and in the fight rather than a wide arena shot; the over-the-shoulder offset is fixed in metres, so it swings wider on screen the closer the camera sits and keeps the sightline clear. Whatever distance is dialled in is then the new default until it is changed again. Aiming pulls in proportionally (about 59% of the chosen distance) so the over-the-shoulder framing holds its relationship at any zoom. While the stick is held in, its vertical axis dollies instead of pitching; yaw still turns the camera.
- **Dynamic camera** (Settings, on by default) — the follow distance rides the player's pace instead of sitting still: about 74% of the dialled-in distance at a standstill or a walk (1.4 m at the default dial), easing out to about 134% at a sprint, dash or under the jetpack (ground speed plus vertical speed, with floors so a hover or a dash reads wide on its own). The shift is continuous and asymmetric — it opens out quickly as you accelerate so the camera is already wide when the ground starts moving, and closes back in slowly, with a short hold after every widening, so stop-start stick work in a firefight never pumps the camera. Aiming keeps its own framing. The right-stick dolly still sets the distance and scales *both* ends of the dynamic range with it, so a camera pulled out stays proportionally pulled out. Turned off, the camera holds the single dialled-in distance exactly as before.
- **Twin energy sabers** (the pale assassin's melee) — the blade is FX, not a model: a
  white-hot core inside two additive red sheaths, casting no shadow, in its own subgroup so
  an authored hilt swap leaves it alone. What sells it as energy rather than a painted
  stick is everything around the geometry: a **point light on the main blade** (parented
  under the blade, so it travels with the swing and costs nothing while stowed — the
  renderer skips invisible subtrees), a **looping hum** that leans up during a swing and
  cuts the moment they are stowed or the shield goes up, an **ignition snap** on the draw,
  and a **crackling energy clash** on contact instead of the staff's metal thud. She also
...
- **Controller assignment follows character-select claims.** Menu use provisionally seats a pad, but the source that opens character select owns player one: either that pad or keyboard/mouse. Each new pad pressing A claims the next human place, and the input seats are aligned to those claims before play. A keyboard-owned player-one seat stays empty in the pad map, allowing the first connected controller to drive player two. Idle devices and unjoined pads cannot shift joined players at match start. A pad keeps its seat during play; losing one pauses instead of shifting another player onto its character. Character select releases a disconnected player's claim and lets a replacement controller take the open place.
- **A character select that grows with the room.** The line holds the players who are here plus exactly one open plinth inviting the next — never a row of dark, unusable slots. Joining opens the next place (up to four); losing a controller closes the line back up, the players past the gap sliding down a place so the seats stay contiguous, with the open plinth returning at the end. The line re-spaces and the stage camera eases back as it grows, name plates track their own plinth, and a click picks whichever plinth is nearest on screen.
- **Readable lighting on dark boards.** Every match has a small neutral ambient floor, with stronger fill on the Lava Flats so black basalt remains visible. Lit saber blades carry a short-range colored light in the hand and in flight; paired sabers can light separate areas when one is thrown. The Darksaber casts no light. On the character-select stage, a soft white backlight begins its slow fade only after each 3D model is ready.
- **Assets warmed ahead, on the screens nobody is playing.** Every heavy file is pulled down during the seconds a player spends in menus, only while the browser reports itself idle and two downloads at a time, so warming never starves the thing actually being waited on. The order is not hand-written per screen: each screen declares what it is made of, and the prefetcher plans from wherever the player is standing — the screen on show downloads at full urgency, the next one behind it, everything further out on idle time, and anything a screen will want but is not showing (the rest of a roster, the waves after the first) rides idle whatever screen declared it. So the title pulls the nine territory cards and the nine planet discs and then the opening fighter; the character select pulls whoever is on a plinth, then the drop screen's portraits and the two files the drop blocks on, then that board's ground, sculpts and later waves on whatever idle time is left. A screen only ever declares what is known — no board is in the plan before one is picked — so nothing speculative is ever queued. Every real load — models through the glTF loader's progress events, textures as they start and finish — reports to one tracker, so anything can ask what is still coming and how far along it is without being handed a list.
- **A drop screen instead of a fade-in.** Starting a match shows the territory's art, the fighters going in and the hostiles waiting there, with a byte-weighted progress bar, and holds until the files that would visibly pop have landed: the players' models, wave one's hostiles, the sky, and whatever textures the board asked for as it built. A territory is never first seen as its procedural stand-in. Procedural fallbacks remain exactly what they were — the answer when a file *cannot* load: a failed load settles like a finished one, so a missing model drops you in with the procedural version rather than stranding the bar. The wait is capped, and after a few seconds A drops you in regardless.
...
**Mount / dismount:** Y (C on keyboard) near a parked vehicle mounts — the same
contextual button as cover, and a vehicle in range wins the press. Y is also the
...
- **RB shields it.** The rider's own force field (the same one `src/fx/shieldfield.ts`
