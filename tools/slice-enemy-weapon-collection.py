"""Clean and split Tripo's five-weapon sheet into independent static GLBs.

Blender: blender -b --python tools/slice-enemy-weapon-collection.py -- SOURCE OUTDIR
The source is never modified. Connected shells determine the five main
...
bm.free()
source.data.update()
bpy.ops.wm.save_as_mainfile(filepath=os.path.join(output_dir, 'enemy_weapon_collection_cleaned.blend'))
...
if len(main) != 5:
    raise RuntimeError(f'Expected five principal weapon shells, found {len(main)}')
main.sort(key=lambda c: sum(v.co.z for v in c) / len(c), reverse=True)
labels = ['enemy_blaster_rifle', 'pirate_boarding_club', 'flame_projector',
          'net_launcher', 'alamite_stone_club']
...
for component in components:
    if component in main:
        ...
        idx = min(range(5), key=lambda i: abs(center - centers[i]))
    assigned[labels[idx]].update(v.index for v in component)
bm.free()

# A full 2K PBR atlas per slice would duplicate 4 MB five times. These props
...
with open(os.path.join(output_dir, 'enemy_weapon_slices_report.json'), 'w') as handle:
    json.dump(report, handle, indent=2)
print('ENEMY_WEAPON_SPLIT', json.dumps(report), flush=True)
