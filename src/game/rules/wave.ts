...
export class WaveRules implements ModeRules {
  ...
  constructor(private g: Game) {
    this.objective = g.board.objective ?? 'Survive 7 waves and two warlords';
  }
  ...
  private clearWave(): void {
    ...
    if (g.wave > this.finalWave) {
      ...
      g.setState('victory');
      g.announce('Territory held', 'This is the Way');
      ...
      g.announce('The champion falls', 'The warlord is watching');
    } else if (g.wave === this.finalWave || (g.wave === this.midBossWave && !this.midBossDown)) {
      // a boss battle rings in on the next bell
      g.setState('break', BREAK);
      g.announce(`Wave ${g.wave} cleared`, 'Something big is coming');
    } else {
      g.setState('break', BREAK);
      g.announce(`Wave ${g.wave} cleared`);
    }
    audio.waveClear();
  }
  ...
  private huntEscalation(dt: number): void {
    ...
    if (!this.huntAnnounced) {
      ...
      g.announce('They are sweeping for you');
    }
  }
  ...
  respawn(p: Player): void {
    ...
    if (INFINITE_LIVES || (g.players.length > 1 && partnerAlive)) {
      ...
      g.setState('defeat');
      g.announce('The hunter has fallen');
    }
  }
  ...
  partyWiped(): void {
    ...
    this.g.announce('The hunters have fallen');
  }

  topLine(): string | null {
    const g = this.g;
    if (this.midBossActive || g.wave > this.finalWave) return g.boss?.bossName ?? 'The warlord';
    return `Wave ${Math.max(g.wave, 1)}`;
  }
  ...
  nextWave(): void {
    ...
    g.announce(
      `Wave ${g.wave}`,
      g.wave === this.finalWave ? `Final wave · ${scattered} hostiles` : `${scattered} hostiles · hunt them down`,
    );
    this.announceDebuts();
    audio.waveStart();
    ...
    if (cacheKind) {
      ...
      g.announce(`Wave ${g.wave}`, 'A covert supply cache is down — crack it open');
    }
  }
