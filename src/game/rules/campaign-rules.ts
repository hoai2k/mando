...
export class CampaignRules implements ModeRules {
  readonly objective = 'Follow the beacon · liberate the territory';
  ...
  update(dt: number): void {
    ...
    if (this.campaign.done && g.state === 'fighting') {
      g.setState('victory');
      g.announce('Territory liberated', 'This is the Way');
      audio.waveClear();
    }
  }
  ...
  respawn(p: Player): void {
    ...
    p.hp = p.maxHp * 0.8;
    g.announce('Back on your feet', 'the beacon waits');
  }
