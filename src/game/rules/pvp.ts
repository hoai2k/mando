...
export class PvpRules implements ModeRules {
  readonly objective = TEXT.banners.objective.pvp;
  ...
  update(): void {
    ...
    for (const p of g.players) {
      ...
      if (killer) {
        ...
        g.announce(TEXT.banners.downs(killer.profile.name, p.profile.name),
        ...
        g.announce(TEXT.banners.isOut(p.profile.name));
      }
      audio.killConfirm();
    ...
    if (g.players.length > 1 && standing.length <= 1) {
      ...
      g.announce(TEXT.banners.takesTheTerritory(winner.profile.name), TEXT.banners.thisIsTheWay);
    }
  }
