...
import { buildMission, MISSION_LAYOUTS, type MissionLevel, type MissionRoom } from '../world/mission-legacy';
 ...
 * the wave game.
 *
 * **This is the previous design**, kept whole behind `?backup=missions`
...
export class LegacyCampaign implements MissionController {
  level: MissionLevel;
  ...
  /**
   * What the objective is called. The room chain always names the room the
   * party is in or heading for; it has no stages, so nothing else can be
   * marked and `exited` below is always empty.
   */
