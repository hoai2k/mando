...
interface MandoConfig {
   ...
   * cycles that slot. Nobody goes without: every fighter is armed in both
   * hands' worth of the word, so a swing and a shot are always one press away.
   */
  ranged?: RangedKind | RangedKind[];
...
/** Every gun this character carries, signature first. */
export function rangedKinds(id: MandoId): RangedKind[] {
  return list(MANDO_ROSTER[id].ranged, 'carbine');
}
...
export const MANDO_ROSTER: Record<MandoId, MandoConfig> = {
  ...
  ventress: {
    ...
    melee: 'sabers', ranged: 'pistols', skin: 0xcdc3ba,
    voice: 'human_f',
  },
  embo: {
...
export function buildMandalorian(id: MandoId, opts: { authored?: boolean } = {}): PlayerCharacter {
  ...
  let gun = guns.get(rangedKinds(id)[0])!;
