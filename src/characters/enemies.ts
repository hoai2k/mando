 ...
 */
function buildKryknaBase(scale: number, bodyColor: number, authored: boolean, creatureId: string): CharacterInstance {
  ...
  let gaitSpeed = 0;
  return {
    root, rig: null, animator: null, height: 1.7 * scale,
    setGait: (speed: number) => { gaitSpeed = speed; },
    cosmetic: (dt, time) => {
      if (!posed) return;
...
export function buildInterceptorDrone(authored = true): CharacterInstance {
  ...
  return {
    root, rig: null, animator: null, height: 1.7,
    cosmetic: (dt, time) => {
      if (!posed) return;
