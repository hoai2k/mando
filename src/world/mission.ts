...
/**
 * An energy gate in a doorway: frame + glow pane, and a physics blocker that
 * exists only while the gate is sealed. The campaign seals a fight room's
 * gates while its waves run and opens them on the clear.
 */
export class Gate {
  pos: THREE.Vector3;
  private box: { min: THREE.Vector3; max: THREE.Vector3 } | null = null;
  ...
  constructor(private board: Board, parent: THREE.Object3D, pos: THREE.Vector3,
    ...
    this.pane = new THREE.Mesh(
      new THREE.PlaneGeometry(GATE_W + 0.4, wallH - 0.6),
      new THREE.MeshBasicMaterial({
        color: accent, transparent: true, opacity: 0.4,
        blending: THREE.AdditiveBlending, depthWrite: false, side: THREE.DoubleSide,
      }));
    this.pane.position.set(pos.x, pos.y + wallH / 2 - 0.2, pos.z);
    this.pane.rotation.y = yaw;
    this.pane.visible = false;
    parent.add(this.pane);
  }
