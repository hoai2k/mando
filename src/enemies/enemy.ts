...
export class Enemy {
  ...
  private senses(dt: number, game: Game): Combatant | null {
    ...
    if (this.team === 0 || this.owner) {
      ...
      // A PvP squad follower is the same soldier escorting its own leader.
      const p = this.owner && this.owner.alive
        ? this.owner
        : game.players.find((pl) => pl.alive && pl.team === this.team)
          ?? game.players.find((pl) => pl.alive) ?? game.players[0];
